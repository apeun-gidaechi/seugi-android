...
fun MessageType.isEmoji(): Boolean = when (this) {
    MessageType.ADD_EMOJI -> true
    MessageType.REMOVE_EMOJI -> true
    else -> false
}

fun MessageType.isSub(): Boolean = when (this) {
    MessageType.SUB -> true
    else -> false
}

fun MessageType.isDeleteMessage(): Boolean = when (this) {
    MessageType.DELETE_MESSAGE -> true
    else -> false
}

fun MessageType.isTransperAdmin(): Boolean = when (this) {
    MessageType.TRANSFER_ADMIN -> true
    else -> false
}
